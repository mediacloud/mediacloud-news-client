...
class TestMediaCloudCollection(TestCase):
    ...
    def test_no_results(self):
        results = self._api.count("madeupword", start_date, end_date)
        assert results == 0

    def test_count_over_time(self):
        results = self._api.count_over_time("coronavirus", start_date, end_date)
    ...
    def test_domain_clause(self):
        domain = "cnn.com"
        results = self._api.sample("trump and canonical_domain:({})".format(domain), start_date, end_date)
        assert len(results) > 0
        for s in results:
            assert s['canonical_domain'] == domain
    ...
    def test_many_domains_clause(self):
        ...
        results = self._api.sample("canonical_domain:({})".format(" OR ".join(domains)), start_date, end_date)
        assert len(results) > 0
        for s in results:
            assert s['canonical_domain'] in domains
    ...
    def test_date_clause(self):
        results = self._api.sample("coronavirus", start_date, end_date)
        assert len(results) > 0
        for r in results:
    ...
    def test_sample(self):
        ...
        for r in results:
            ...
            assert 'canonical_domain' in r
            assert 'article_title' in r
            assert len(r['article_title']) > 0
    ...
    def test_domain_and_url_search(self):
        ...
        mountpleasant_dv = self._api.count("canonical_domain:dailyvoice.com AND original_url:*mountpleasant*", start_date, end_date)
        assert all_dv > mountpleasant_dv
        assert mountpleasant_dv > 0
        # does searching by partial URL path work?
        sample_mountplesant = self._api.sample("canonical_domain:dailyvoice.com AND original_url:*mountpleasant*", start_date, end_date)
